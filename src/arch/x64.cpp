...
#include "msga/arch/x64.h"
#include "hde64.h"
#include "msga/arch/x86_64.h"
#define ABS_JMP_SIZE 15

namespace msga{
    namespace arch{
        bool x64::generate(io::base&io,addr_t addr,std::vector<code>&generated,std::vector<code>&backup,addr_t origin=0){
            hde64s hde;
            ...
            while(off_addr<ABS_JMP_SIZE){
                size_t width=hde64_disasm(buffer+offset,&hde);
